...
class sweetnoteStorage {
    static let storage : sweetnoteStorage = sweetnoteStorage()
    ...
    func setManagedContext(managedObjectContext: NSManagedObjectContext) {
        ...
        let notes = sweetnoteCoreDataHelper.readNotesFromCoreData(fromManagedObjectContext: self.managedObjectContext)
        currentIndex = sweetnoteCoreDataHelper.count
        for (index, note) in notes.enumerated() {
            noteIndexToIdDict[index] = note.noteId
        }
    }
    
    func addNote(noteToBeAdded: sweetnote) {
        if managedContextHasBeenSet {
            // add note UUID to the dictionary
            noteIndexToIdDict[currentIndex] = noteToBeAdded.noteId
            sweetnoteCoreDataHelper.createNoteInCoreData(
    ...
    func removeNote(at: Int) {
        if managedContextHasBeenSet {
            ...
            sweetnoteCoreDataHelper.deleteNoteFromCoreData(
    ...
    func readNote(at: Int) -> sweetnote? {
        if managedContextHasBeenSet {
            // check input index
            if at < 0 || at > currentIndex-1 {
                // TODO error handling
            ...
            // get note UUID from the dictionary
            let noteUUID = noteIndexToIdDict[at]
            let noteReadFromCoreData: sweetnote?
            noteReadFromCoreData = sweetnoteCoreDataHelper.readNoteFromCoreData(
    ...
    func changeNote(noteToBeChanged: sweetnote) {
        if managedContextHasBeenSet {
            ...
            if noteToBeChangedIndex != nil {
                sweetnoteCoreDataHelper.changeNoteInCoreData(
